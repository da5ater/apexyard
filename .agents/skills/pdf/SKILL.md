...
Read `workspace_dir` and `projects_dir` from `.codex/hooks/_lib-portfolio-paths.sh` so split-portfolio adopters resolve to the sibling private repo transparently:
...
`.codex/project-config.defaults.json` ships a `pdf` block:
...
Adopters override in `.codex/project-config.json`:
...
- `.agents/skills/process/lint.sh` — graceful-degrade-on-missing-dep pattern that this skill mirrors for converter detection
- `.agents/skills/_lib-mermaid-lint.sh` — same pattern, npx-fallback case
