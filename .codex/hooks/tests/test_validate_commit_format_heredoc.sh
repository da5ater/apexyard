...
HOOK_SRC="$SRC_ROOT/.codex/hooks/validate-commit-format.sh"
...
make_sandbox() {
  ...
  mkdir -p "$sb/.codex/hooks"
  cp "$HOOK_SRC" "$sb/.codex/hooks/validate-commit-format.sh"
  if [ -f "$LIB_CFG" ]; then cp "$LIB_CFG" "$sb/.codex/hooks/_lib-read-config.sh"; fi
  if [ -f "$DEFAULTS" ]; then cp "$DEFAULTS" "$sb/.codex/project-config.defaults.json"; fi
  chmod +x "$sb/.codex/hooks/validate-commit-format.sh"
  echo "$sb"
}
...
run_case() {
  ...
  got_stderr=$(cd "$sb" && echo "$input" | bash .codex/hooks/validate-commit-format.sh 2>&1 >/dev/null)
  got_rc=$?
  rm -rf "$sb"
