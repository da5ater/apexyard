...
# .codex/hooks/_lib-portfolio-paths.sh + the schema validity of the
...
MD
    ...
    mkdir -p .codex/hooks
    cp "$LIB_SRC" .codex/hooks/_lib-portfolio-paths.sh
    cp "$CONFIG_LIB_SRC" .codex/hooks/_lib-read-config.sh
    cp "$DEFAULTS_SRC" .codex/project-config.defaults.json
...
run_case() {
  ...
  out=$(
    ...
    . .codex/hooks/_lib-read-config.sh
    # shellcheck source=/dev/null
    . .codex/hooks/_lib-portfolio-paths.sh
...
inner=$(
  source .codex/hooks/_lib-read-config.sh
  source .codex/hooks/_lib-portfolio-paths.sh
  portfolio_clear_cache
  portfolio_agent_routing >/dev/null   # populates cache
  cat > .codex/project-config.json <<JSON
