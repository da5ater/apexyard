#!/bin/bash
# Pre-commit + pre-push drift guard for .codex/agents/*.md routing.
#
...
#   - For each staged (commit) or to-be-pushed (push) .codex/agents/*.md
#     file:
#       1. Read its current model: line
#       2. Compare against the framework default — first preference is
#          the snapshot at .codex/agents/.framework-defaults.json
#          (written by the sync hook on the most recent SessionStart);
#          fallback is `git show dev:.codex/agents/<name>.md` (works
...
#   1. Snapshot at .codex/agents/.framework-defaults.json (written by
#      apply-agent-routing.sh on the most recent SessionStart)
#   2. dev:.codex/agents/<name>.md frontmatter
#   3. upstream/main:.codex/agents/<name>.md frontmatter
#   4. HEAD:.codex/agents/<name>.md frontmatter (last-resort — handles
...
framework_default_for() {
  ...
  if git -C "$ROOT" rev-parse --verify dev >/dev/null 2>&1; then
    m=$(git -C "$ROOT" show "dev:.codex/agents/${agent_name}.md" 2>/dev/null \
  ...
  if git -C "$ROOT" rev-parse --verify upstream/main >/dev/null 2>&1; then
    m=$(git -C "$ROOT" show "upstream/main:.codex/agents/${agent_name}.md" 2>/dev/null \
        | awk '/^---/{count++; next} count==1 && /^model:/ {sub(/^model:[[:space:]]*/, ""); print; exit}')
    [ -n "$m" ] && { echo "$m"; return 0; }
  fi

  # 4. HEAD baseline (handles brand-new agent file in this commit).
  m=$(git -C "$ROOT" show "HEAD:.codex/agents/${agent_name}.md" 2>/dev/null \
      | awk '/^---/{count++; next} count==1 && /^model:/ {sub(/^model:[[:space:]]*/, ""); print; exit}')
  [ -n "$m" ] && echo "$m"
}
...
.codex/rules/leak-protection.md for the broader leak-prevention pattern.
MSG
