...
# in AGENTS.md, workflows/sdlc.md, or .codex/rules/workflow-gates.md.
#
# Active tickets are declared by the /start-ticket skill. The marker
# layout is three-tier (apexyard#41 + #513):
#
#   ops_root/.codex/session/tickets/<project>/<branch>  ← per-worktree (#513)
#   ops_root/.codex/session/tickets/<project>           ← per-project (#41)
...
#   0. If the file's repo is on a git worktree branch (or CODEX_WORKTREE_BRANCH
...
#   - any *.md file (READMEs, AGENTS.md, rule docs, AgDRs)
#   - anything under docs/
...
if [ "$TOOL_NAME" = "Bash" ]; then
  ...
  # exemptions (.codex/, docs/, *.md). If extraction fails, FILE_PATH
  ...
  # (`$CEO`, `${marker}` — unresolvable, in practice a .codex/ scratch path). A
...
if [ -n "$REL_PATH" ]; then
  case "$REL_PATH" in
    .codex/*|.claude|*/.codex/*|*/.claude) exit 0 ;;
    docs/*|docs|*/docs/*|*/docs) exit 0 ;;
    TODO.md|README.md|MEMORY.md|AGENTS.md) exit 0 ;;
  esac
...
# marker at .codex/session/active-bootstrap with the skill name on
...
BOOTSTRAP_MARKER="$MARKER_HOME/.codex/session/active-bootstrap"
if [ -f "$BOOTSTRAP_MARKER" ]; then
  ...
  if [ -n "$active_bootstrap" ]; then
    ...
    if [ -f "$MARKER_HOME/.codex/hooks/_lib-read-config.sh" ]; then
      # shellcheck source=/dev/null
      . "$MARKER_HOME/.codex/hooks/_lib-read-config.sh"
...
if [ -n "$PROJECT" ]; then
  ...
  if [ -n "$WT_BRANCH" ]; then
    ...
    PER_WORKTREE_MARKER="$MARKER_HOME/.codex/session/tickets/$PROJECT/$SAFE_BRANCH"
...
PER_PROJECT_MARKER=""
if [ -n "$PROJECT" ]; then
  PER_PROJECT_MARKER="$MARKER_HOME/.codex/session/tickets/$PROJECT"
...
FALLBACK_MARKER="$MARKER_HOME/.codex/session/current-ticket"
...
Exempt paths (no ticket required): .codex/, docs/, projects/*/docs/, *.md
MSG
exit 2
