...
#
# The marker file at .codex/session/pending-reviews/<pr> is also read by
# the merge-gate hook so a PR cannot be merged without a corresponding Rex
# approval file at .codex/session/reviews/<pr>-rex.approved.

INPUT=$(cat)
TOOL_NAME=$(echo "$INPUT" | jq -r '.tool_name // empty' 2>/dev/null)
COMMAND=$(echo "$INPUT" | jq -r '.tool_input.command // empty' 2>/dev/null)
# The `gh` output may surface at .tool_response.stdout (newer harness,
# Codex 2.x+), .tool_response.output (older 1.x), or .tool_response as
...
mkdir -p "${REPO_ROOT:-.}/.codex/session/pending-reviews"
if [ -n "$PR_NUMBER" ]; then
  echo "${PR_URL}" > "${REPO_ROOT:-.}/.codex/session/pending-reviews/${PR_NUMBER}"
fi
...
and .codex/rules/pr-workflow.md.

${AGENT_INSTRUCTION}

The merge-gate hook will block \`gh pr merge\` for this PR until a Rex approval
file exists at .codex/session/reviews/${PR_NUMBER:-<pr>}-rex.approved.
