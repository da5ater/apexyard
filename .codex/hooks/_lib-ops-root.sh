...
#
# Hooks that write or read framework session state (`.codex/session/*`)
...
#   - CODEX_SESSION_ID unset   → no pin lookup, walk-up only
#
...
resolve_ops_root() {
  ...
  if [ -z "${APEXYARD_OPS_DISABLE_PIN:-}" ] && [ -n "${CODEX_SESSION_ID:-}" ]; then
    local pin_dir="${APEXYARD_OPS_PIN_DIR:-$HOME/.codex/apexyard}"
    local pin_file="$pin_dir/ops-root-${CODEX_SESSION_ID}"
    if [ -f "$pin_file" ]; then
      local pinned=""
